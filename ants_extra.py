    ...
    def simulate_next_ant(self):
        if self.current_ant < self.num_ants:
            ...
            for i in range(len(path) - 1):
                delta_pheromone = 0.2 / total_distance  # Proportional to path quality
                self.pheromones[path[i], path[i + 1]] += delta_pheromone
                self.pheromones[path[i + 1], path[i]] += delta_pheromone

            # Update the best path (elitist solution)
            if not hasattr(self, "best_path") or total_distance < self.best_distance:
                self.best_path = path
                self.best_distance = total_distance

            # Apply elitist reinforcement to the best path
            elite_reinforcement = 0.5  # Elitist pheromone weight
            for i in range(len(self.best_path) - 1):
                self.pheromones[self.best_path[i], self.best_path[i + 1]] += elite_reinforcement
                self.pheromones[self.best_path[i + 1], self.best_path[i]] += elite_reinforcement

            # Evaporate pheromones
            evaporation_rate = 0.05  # Evaporation coefficient
            self.pheromones *= (1 - evaporation_rate)

            # Clip pheromone values to avoid extremes
            ...
            # All ants have completed their paths
            self.next_ant_button.setText("Все муравьи завершили!")
            self.current_path = []  # Clear the red path
            self.canvas.update_graph(self.pheromones, self.current_path, self.current_ant)
